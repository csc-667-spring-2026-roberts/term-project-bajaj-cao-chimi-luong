...
export async function up(pgm: MigrationBuilder): Promise<void> {
  ...
  pgm.createTable("games", {
    ...
    status: {
      ...
      references: "game_status_types(status)",
    },
  ...
  pgm.createTable("game_cards", {
    ...
    location: {
      ...
      notNull: true,
      default: "DECK",
      references: "location_types(location)",
    },
    position: { type: "int" },
  ...
  pgm.addConstraint(
    "game_cards",
    "hand_user_check",
    `CHECK ((location = 'HAND' AND user_holding IS NOT NULL) OR (location <> 'HAND' AND user_holding IS NULL))`,
  );
  ...
  pgm.createTable("messages", {
    ...
    initiating_reason: {
      type: "varchar(50)",
      notNull: true,
      references: "initiating_reason_types(reason)",
    },
