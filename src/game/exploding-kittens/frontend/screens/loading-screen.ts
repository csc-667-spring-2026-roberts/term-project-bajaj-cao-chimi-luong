export class LoadingScreen {
  ...
  public play(): void {
    ...
    setTimeout(() => { this.renderCat(); }, welcomeFinishDelay);

    const catDelay = welcomeFinishDelay + this.titleText.length * this.baseDelay + 5000;
    setTimeout(() => {this.loadPlayButton()}, catDelay);
  ...
  private renderCat(): void{
    ...
    setTimeout(() => {this.setCatState(cat, 'roll')}, 4000)
    setTimeout(() => {this.setCatState(cat, 'pull')}, 8000);
  }
  ...
  private triggerWipe(): void {
    ...
    this.wipe.addEventListener('animationend', () => {
